...
### Problem Definition
To help determine the role electric vehicle sales have in curtailing CO2 emissions, this project will attempt to develop a correlation between yearly electric vehicle sales and CO2 transportation emissions to predict CO2 emissions based on future electric vehicle sales. Due to increasing sales of other gas-powered vehicles, the direct relationship between current EV sales on transportation CO2 emissions is still unknown. Similarly, there is no data on whether using electric vehicles is a best way to mitigate CO2 emission. This project will attempt to analyze the impact of advanced technological vehicles (HEV, EV, PHEV) by creating a linear regression model mapping CO2 emission levels to ATV sales and non-ATV sales of the same year. A risk level will then be assigned to each state to determine what states would benefit the most from an increase in the ratio between ATVs to internal combustion engine vehicle sales. It will move on to identifying the most important emission mitigation activity and utilizing the compiled data to predict the behavior of the two most high-risk states regarding CO2 emission level and their relation to gas prices in the future
